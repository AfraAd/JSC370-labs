...
curl https://github.com/AfraAd/JSC370-labs/blob/main/lab05-wrangling-gam.Rmd --output README.Rmd
...
# Step 5
git remote add origin git@github.com:AfraAd/JSC370-labs
...
station_med[wind.sp_dist == 0 & atm.press_dist == 0]
```

### Ans
The following tables show which stations have median temperature, wind speed, and atmospheric pressure around the overall median. Weather station 725975 have median wind speed and atmospheric pressure at the overall median, otherwise there are no more coinciding stations.
...
leaflet() |>
  ...
  addCircles(
    ...
    radius =400, color="blue"
  )|>
  addCircles(
    data=id_station,
    lat = ~lat, lng = ~lon, popup = "geographic mid station",
    radius =400, color="red"
  )
```
...
```{r, results='asis'}
kable(summary_table, booktabs = T) |>
...
```
# Ans.
Both models have same significant coefficients and ANOVA test p-value, so I will go for the linear regression as it is simpler. The spline model is sensitive to the extreme atmospheric pressure values, so the model is over fitting the training data; on the other hand, the linear regression is less sensitive to the out-liars and does not appear to over fit the data based on the graph.
